...
describe('Teste do componente <Pokedex.js />', () => {
  ...
  it('Verifica se é exibido o próximo Pokémon da lista quando o botão "Próximo pokémon" é clicado', () => {
    renderWithRouter(<App />);
    const prevPokemonName = screen.getByTestId('pokemon-name');
    const nextButton = screen.getByRole('button', { name: /próximo pokémon/i });
    userEvent.click(nextButton);
    const nextPokemonName = screen.getAllByTestId('pokemon-name');
    expect(prevPokemonName).not.toEqual(nextPokemonName);
  });
  it('Verifica se é mostrado apenas um Pokémon por vez.', () => {
    renderWithRouter(<App />);
    const pokemonRendered = screen.getAllByTestId('pokemon-name');
  ...
  it('Verifica se a Pokédex tem os botões de filtro.', () => {
    ...
    const filter = screen.getByTestId('pokemon-type');
  ...
  it('Verifica se a Pokédex contém um botão para resetar o filtro', () => {
    ...
    const prevType = screen.getByTestId('pokemon-type').innerHTML;
    const nextButton = screen.getByRole('button', { name: /próximo pokémon/i });
    userEvent.click(nextButton);
    const newType = screen.getByTestId('pokemon-type').innerHTML;

...
describe('Req 6 Pokdemon Details test', () => {
  test('Checks if pokemon info is rendered on screen', () => {
  ...
  test('checks if a map is rendered on screen', () => {
    renderWithRouter(<App />);
  ...
  test('Checks if user can add a pokemon as a favorite', () => {
    renderWithRouter(<App />);
    const details = screen.getByRole('link', { name: /More Details/i });
    expect(details).toBeInTheDocument();
    userEvent.click(details);
